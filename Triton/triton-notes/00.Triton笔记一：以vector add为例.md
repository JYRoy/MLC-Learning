...
还有machine-dependent的passes，根据论文中的内容，主要有四个（可能和现在有差异，目前具体的实现需要之后再细看）：
...
例如Shared Memory Allocation pass用于：

1. 计算密集型的block级别OP（如tl.dot）的操作数存储到到shared memory中；
2. 使用liveness analysis技术进行allocated/synchronized；

通过分析感兴趣的变量的生存周期，利用线性时间存储分配算法来完成共享内存的分配：
