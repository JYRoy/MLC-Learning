...
对于MM的优化，起始大概的思路都差不多，因为总的计算量一定是不变的，所以能做的事情就是尽可能地使用shared memory，提高cache hit rate.
...
为了保证L2 Cache的命中率，这些block的顺序非常重要，但不幸的是，对于简单的行主序不会切分它，一个原始的triton实现如下：
...
2. strides：和我们在torch中看到的stride是同一个概念，strid0对应0维+1需要的stride，stride1对应1维+1需要的stride；
...
这是在vector add用例中就已经看过的内容。在kernel内之所以可以写成只处理部分内容的形式，就是program在发挥它的作用。
...
前面说了program理解为循环，每一个index对应了输出C矩阵中的一部分，这一部分就是我们说的block，block的大小是${BLOCK\_SIZE\_M} \times {BLOCK\_SIZE\_N}$，则循环的次数就是$\frac{M}{BLOCK\_SIZE\_M} \times \frac{N}{BLOCK\_SIZE\_N}$。这里也就是我们上面看到的grid的计算公式，即grid=81。
